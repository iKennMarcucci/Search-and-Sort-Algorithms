...
// 	ListaOrdenada2 := InsercionInv(ListaDesordenada2)
// 	fmt.Println(ListaOrdenada2)
// }

func main() {
	n := 1000 //definir tamaño del arreglo
	...
	arregloOrdenado := Insercion(arregloDeNumeros)
	...
	arregloOrdenadoMm := InsercionInv(arregloDeNumeros)
...
func Insercion(ListaDesordenada []int) []int {
	...
	fmt.Printf("Iteraciones de ordenamiento por Insercion: %d \n", iter)
	return ListaDesordenada
}

func InsercionInv(ListaDesordenada []int) []int {
	var auxiliar int
	for i := 1; i < len(ListaDesordenada); i++ {
		auxiliar = ListaDesordenada[i]
		for j := i - 1; j >= 0 && ListaDesordenada[j] < auxiliar; j-- {
